...
####suggested title by JPG


\textbf{Simulating real-life COVID-19 epidemics: lessons from calibrating the Covasim model across 35 countries}


####New abstract suggested by JPG

\textbf{The COVID-19 pandemic has highlighted the importance of mathematical modelling to make predictions and inform decision making. Calibrating settings-specific epidemics can require prior information on specific parameters or can lead to overfitting to data for unrealistic and large set of parameters. 

This work showcases how different country epidemics can be replicated using Covasim, stochastic agent-based model, by focused sweeping of the parameter space during the calibration process. Across 35 different countries, we methodologically and hierarchically search the n-dimensional parameter space and derive the minimum number of parameters required to reach global stability for each country. We compare our findings across countries where school-interventions were imposed during the first wave of COVID-19 with those without such interventions, quantifying the fit across the different settings. For the calibration, we use publicly available data and Optuna hyperparametrisation algorithm.

Our results suggest the epidemic trajectories across the 35 countries can be replicated with variations in three key parameters: the probability of infection, the number of infected individuals at the onset of the epidemic and the daily probability of testing for COVID-19. Our results suggest that the model fit was better in countries where school-interventions were imposed than in those without.}

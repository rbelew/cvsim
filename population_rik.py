...
def make_people(sim, save_pop=False, popfile=None, die=True, reset=False, verbose=None, **kwargs):
	...
	if sim.people and not reset:
		...
		if pop_type in ['random', 'clustered', 'hybrid','educlevel']:
...
def make_randpop(sim, use_age_data=True, use_household_data=True, sex_ratio=0.5, microstructure=False):
	...
	if location is not None:
		...
		if use_age_data:
			try:
				# NB: can't test if sim['age_dist'] != None
				# ValueError: The truth value of an array with more than one element is ambiguous. Use a.any() or a.all()
				if 'age_dist' in sim.pars and sim['age_dist'].any():
					age_data = sim['age_dist']
				else:
	...
	elif microstructure == 'educlevel':	contacts, layer_keys, _ = make_educLevel_contacts(pop_size, ages, sim['contacts'])
	else:
